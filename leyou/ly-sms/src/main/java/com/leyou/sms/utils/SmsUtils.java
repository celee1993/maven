...
public class SmsUtils {
    ...
    private static final String KEY_PREFIX = "sms:limit:";
    ...
    public SendSmsResponse sendSms(String phoneNumber,  String signName,String templateCode, String templateParam){
        ...
        if (StringUtils.isNotBlank(lastTime)) {
            //限流
            log.info("[短信服务]发送短信太频繁,请稍后再试，手机号码：{}",phoneNumber);
            return null;
        }
        try {
            //可自助调整超时时间
            ...
            //发送成功后 将手机号存入redis
            template.opsForValue().set(key,phoneNumber,1, TimeUnit.MINUTES);
            return sendSmsResponse;
        } catch (ClientException e) {
            log.info("[短信服务]发送短信失败，phoneNumber:{}",phoneNumber);
            return null;
        }
    }
}
